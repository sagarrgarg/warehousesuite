...
class ValidationResult:
	...
	def __init__(self, is_valid: bool = True, errors: list[dict] | None = None):
		self.is_valid = is_valid
		self.errors = errors or []

	def add_error(self, message: str, field: str | None = None, code: str | None = None):
		"""Add an error to the validation result"""
		self.errors.append({"message": message, "field": field, "code": code})
		self.is_valid = False
...
def validate_warehouse_transfer_data(
	source_warehouse: str, target_warehouse: str, items: list[dict], company: str
) -> ValidationResult:
	"""
...
def validate_stock_availability(item_code: str, warehouse: str, qty: float, uom: str) -> ValidationResult:
	...
	try:
		...
		result.add_error(f"Error checking stock availability: {e!s}", "stock_check", "STOCK_CHECK_ERROR")
...
def validate_concern_data(concern_data: dict) -> ValidationResult:
	...
	if concern_data.get("concern_type") and concern_data["concern_type"] not in valid_concern_types:
		result.add_error(
			f"Invalid concern type. Must be one of: {', '.join(valid_concern_types)}",
			"concern_type",
			"INVALID_CONCERN_TYPE",
		)
	...
	if concern_data.get("priority") and concern_data["priority"] not in valid_priorities:
		result.add_error(
			f"Invalid priority. Must be one of: {', '.join(valid_priorities)}", "priority", "INVALID_PRIORITY"
		)
...
def validate_transfer_receive_data(stock_entry_name: str, items_data: list[dict]) -> ValidationResult:
	...
	try:
		...
		frappe.logger().error(f"Error in validate_transfer_receive_data: {e!s}")
		result.add_error(f"Validation error: {e!s}", "validation_error", "VALIDATION_ERROR")
		return result
...
def validate_receive_item(item: dict, stock_entry: Any, index: int = 0) -> ValidationResult:
	...
	if item.get("item_code"):
		...
		if not original_item:
			result.add_error(
				f"Item {item['item_code']} not found in original stock entry",
				"item_code",
...
def get_already_received_quantity(stock_entry_name: str, item_code: str) -> float:
	...
	try:
		...
		frappe.logger().error(f"Error getting already received quantity: {e!s}")
		return 0.0
...
def get_stock_quantity(item_code: str, warehouse: str) -> float:
	...
	try:
		...
		frappe.logger().error(f"Error getting stock quantity for {item_code} in {warehouse}: {e!s}")
		return 0.0
...
def get_uom_conversion_factor(item_code: str, from_uom: str, to_uom: str) -> float:
	...
	try:
		...
		conversion = frappe.db.get_value(
			"UOM Conversion Detail", {"parent": item_code, "uom": to_uom}, "conversion_factor"
		)
...
def create_api_response(validation_result: ValidationResult, data: dict | None = None) -> dict:
	"""

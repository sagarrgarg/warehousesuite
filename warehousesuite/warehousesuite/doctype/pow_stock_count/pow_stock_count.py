...
from frappe.utils import flt, format_datetime, get_datetime, getdate, now_datetime
...
class POWStockCount(Document):
	...
	def check_draft_limitation(self):
		...
		if self.status == "Draft" and not self.flags.ignore_draft_validation:
			...
			if existing_drafts:
				frappe.throw(
					_(
						"A draft stock count already exists for warehouse {0} in this session. Please complete or delete the existing draft first."
	...
	def convert_to_stock_reconciliation(self):
		...
		try:
			...
			frappe.logger().error(f"Error converting stock count to reconciliation: {e!s}")

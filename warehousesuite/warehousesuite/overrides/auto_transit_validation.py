...
def auto_set_transit_for_material_transfer(doc, method):
	...
	# Skip revert transfers from stock concerns
	if getattr(doc, "pow_stock_concern", None):
		return

	# Skip receive entries — they have outgoing_stock_entry set
	if getattr(doc, "outgoing_stock_entry", None):
		return
...
def _get_wmsuite_settings():
	...
	try:
		...
		return {"auto_set_transit": getattr(settings_doc, "auto_set_transit", 1)}
	except Exception as e:
		# Log the error but don't break the functionality
		frappe.log_error(f"Error getting WMSuite Settings: {e!s}", "WMSuite Auto Transit")

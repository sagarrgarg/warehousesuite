...
def get_bom_for_item(item_code, allowed_warehouses=None, bom_no=None):
	...
	for row in bom.items:
		item_info = frappe.db.get_value("Item", row.item_code, ["item_name", "stock_uom"], as_dict=True) or {}
...
def create_work_order(
	...
	if not frappe.get_list(
		"BOM", filters={"name": bom_no, "item": production_item, "docstatus": 1}, limit_page_length=1
	):
...
def get_work_order_materials(wo_name, allowed_warehouses=None):
	...
	for item in wo.required_items:
		...
		if wip_wh:
			wip_avail = flt(
				frappe.db.get_value("Bin", {"item_code": item.item_code, "warehouse": wip_wh}, "actual_qty")
				or 0
			)
	...
	per_completed = min(100, round(flt(wo.produced_qty) / qty * 100))
	per_available = (
		min(100, round(sum(item_fill_ratios) / len(item_fill_ratios) * 100)) if item_fill_ratios else 100
	)
...
def transfer_materials_for_manufacture(wo_name, items, batch_serial_data=None):
	...
	for item_data in items:
		...
		if not src:
			shortfalls.append(_("{0}: no source warehouse selected").format(item_code))
			continue
...
def get_material_shortfall(wo_name):
	...
	for item in wo.required_items:
		...
		item_info = (
			frappe.db.get_value("Item", item.item_code, ["item_name", "stock_uom"], as_dict=True) or {}
		)
...
def direct_manufacture(
	...
	if item_substitutions:
		subs = (
			frappe.parse_json(item_substitutions)
			if isinstance(item_substitutions, str)
			else item_substitutions
		)
	...
	if subs:
		for item_row in se.items:
			...
			if substitute not in alt_codes:
				frappe.throw(_("{0} is not a valid alternative for {1}").format(substitute, original))

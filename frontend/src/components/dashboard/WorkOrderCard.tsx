...
export default function WorkOrderCard({ wo, onClick }: WorkOrderCardProps) {
  const stripeColor = STATUS_COLORS[wo.status] ?? 'bg-slate-500'
  const statusTextColor = STATUS_TEXT[wo.status] ?? 'text-slate-400'
  ...
  return (
    <button
      ...
      className="w-full text-left flex items-stretch border-b border-slate-700 hover:bg-slate-750 active:bg-slate-700 transition-colors cursor-pointer touch-manipulation"
    >
      {/* Status stripe */}
      ...
      <div className="flex-1 px-2.5 py-2 min-w-0">
        ...
        <div className="flex items-center justify-between gap-2 mb-0.5">
          <span className="text-[10px] font-mono text-slate-400 truncate">{wo.name}</span>
        ...
        <div className="flex items-baseline justify-between gap-2 mb-1.5">
          <span className="text-[11px] font-semibold text-white truncate">{wo.item_name || wo.production_item}</span>
          <span className="text-[11px] font-bold tabular-nums text-slate-200 shrink-0">{wo.qty}</span>
        </div>

...
import { Clock, AlertTriangle, MapPin, Users, Edit3, FileText, TrendingUp, PackageX, Star } from 'lucide-react'
...
export default function SOAnalyticsTab({ powProfileName }: { powProfileName: string }) {
	...
	return (
		<div className="max-w-6xl mx-auto p-3 space-y-4">
			...
			<div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
				<MetricCard label="Open Orders" value={totalPending.count} sub="pending fulfillment" icon={<FileText className="w-4 h-4 text-cyan-500" />} />
				<MetricCard label="Avg to Deliver" value={formatHrs(turnaround30d?.avg_hrs_to_dn ?? null)} sub="last 30 days" icon={<Clock className="w-4 h-4 text-emerald-500" />} valueColor={timeColor(turnaround30d?.avg_hrs_to_dn ?? null, 48, 120)} />
				<MetricCard label="Suggest Close" value={a.nearly_complete_count} sub=">80% done & >14d old" icon={<AlertTriangle className="w-4 h-4 text-amber-500" />} valueColor={a.nearly_complete_count > 0 ? 'text-amber-600' : 'text-emerald-600'} />
				<MetricCard label="Can Be Closed" value={a.ignore_count} sub=">95% delivered" icon={<TrendingUp className="w-4 h-4 text-red-500" />} valueColor={a.ignore_count > 0 ? 'text-red-600' : 'text-slate-400'} />
			</div>

			{/* Pending by status */}
			{a.pending_summary.length > 0 && (
				<section>
					<H icon={<FileText className="w-3.5 h-3.5 text-blue-500" />} title="Open Orders by Status" />
			...
			{/* Turnaround */}
			<section>
				<H icon={<Clock className="w-3.5 h-3.5 text-cyan-500" />} title="Order Turnaround Time" />
							...
							<div key={period} className={`flex items-center gap-3 px-3 py-2.5 ${i > 0 ? 'border-t border-slate-100 dark:border-slate-800' : ''}`}>
								...
								<div className="flex-1 grid grid-cols-3 gap-2">
									<div className="text-center">
										...
										<p className={`text-sm font-bold tabular-nums ${timeColor(row?.avg_hrs_to_dn ?? null, 48, 120)}`}>{formatHrs(row?.avg_hrs_to_dn ?? null)}</p>
									</div>
									<div className="text-center">
										<p className="text-[8px] text-slate-400 uppercase tracking-wider mb-0.5">DN → Invoice</p>
										<p className={`text-sm font-bold tabular-nums ${timeColor(row?.avg_hrs_dn_to_si ?? null, 24, 72)}`}>{formatHrs(row?.avg_hrs_dn_to_si ?? null)}</p>
									</div>
									<div className="text-center">
										<p className="text-[8px] text-slate-400 uppercase tracking-wider mb-0.5">Order → Invoice</p>
										<p className={`text-sm font-bold tabular-nums ${timeColor(row?.avg_hrs_to_si ?? null, 72, 168)}`}>{formatHrs(row?.avg_hrs_to_si ?? null)}</p>
									</div>
								</div>
							</div>
						)
			...
			{a.unfulfillment.length > 0 && (
				<section>
					<H icon={<PackageX className="w-3.5 h-3.5 text-red-500" />} title="Item-Level Unfulfillment Rate" />
					<div className="bg-white dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-700">
							...
							return (
								<div key={period} className={`flex items-center gap-3 px-3 py-2.5 ${i > 0 ? 'border-t border-slate-100 dark:border-slate-800' : ''}`}>
									...
									<div className="flex-1">
										...
										<p className="text-[9px] text-slate-400 mt-0.5 tabular-nums">{row?.unfulfilled_lines ?? 0} of {row?.total_lines ?? 0} lines not fully delivered</p>
									</div>
								</div>
							)
						})}
			...
			{a.nearly_complete.length > 0 && (
				<section>
					<H icon={<AlertTriangle className="w-3.5 h-3.5 text-amber-500" />} title={`${a.nearly_complete_count} Orders to Close (>80% done, >14 days old)`} />
					<div className="space-y-1.5">
						{a.nearly_complete.slice(0, 15).map(nc => (
							<div key={nc.name} className="bg-white dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-700 px-3 py-2 flex items-center gap-2">
								<div className="flex-1 min-w-0">
									...
									<p className={`text-[9px] font-bold mt-0.5 ${nc.days_open > 30 ? 'text-red-500' : 'text-slate-400'}`}>{nc.days_open} days old</p>
								</div>
						...
						{a.nearly_complete.length > 15 && (
							<p className="text-[10px] text-slate-400 text-center py-1">+{a.nearly_complete.length - 15} more</p>
						)}
					</div>
				</section>
			)}
			...
			{a.top_skus.length > 0 && (
				<section>
					<H icon={<Star className="w-3.5 h-3.5 text-amber-500" />} title="Top Selling Items" />
			...
			{/* Modifications */}
			<section>
				<H icon={<Edit3 className="w-3.5 h-3.5 text-violet-500" />} title="Order Changes" />
				<div className="bg-white dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-700">
					{['7d', '30d', '6m'].map((period, i) => {
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
				<section>
					<H icon={<MapPin className="w-3.5 h-3.5 text-emerald-500" />} title="Top Cities" />
					<RankList rows={a.top_cities.map(c => ({ label: c.city, total: c.order_count, d30: c.count_30d, d7: c.count_7d }))} accentColor="text-emerald-600" />
				</section>
				<section>
					<H icon={<Users className="w-3.5 h-3.5 text-blue-500" />} title="Top Customers" />
					<RankList rows={a.top_customers.map(c => ({ label: c.customer_name, total: c.order_count, d30: c.count_30d, d7: c.count_7d }))} accentColor="text-blue-600" />
				</section>
			</div>
		</div>
	)
...
function H({ icon, title }: { icon: React.ReactNode; title: string }) {
	return <h3 className="text-xs font-bold text-slate-700 dark:text-slate-300 mb-2 flex items-center gap-1.5">{icon} {title}</h3>
}
...
function MetricCard({ label, value, sub, icon, valueColor }: { label: string; value: string | number; sub: string; icon: React.ReactNode; valueColor?: string }) {
	return (
		<div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 p-3">
			<div className="flex items-center gap-1.5 mb-1.5">{icon}<span className="text-[9px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">{label}</span></div>
...
function ProgressPill({ label, pct }: { label: string; pct: number }) {
	...
	return <span className={`text-[9px] font-bold rounded px-1.5 py-0.5 tabular-nums ${color}`}>{label} {rounded}%</span>
}

interface RankRow { label: string; total: number; d30: number; d7: number }

function RankList({ rows, accentColor }: { rows: RankRow[]; accentColor: string }) {
	...
	return (
		<div className="bg-white dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800">
			...
			{rows.length === 0 && <div className="px-3 py-6 text-center text-xs text-slate-400">No data</div>}
		</div>
	)
}

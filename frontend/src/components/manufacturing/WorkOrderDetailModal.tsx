...
function ItemRow({
  ...
  const stripeColor = STOCK_STATUS_COLORS[item.stock_status] ?? 'bg-slate-500'
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-4 gap-1 mt-1.5 text-[9px]">
            <div>
              ...
              <p className="font-bold tabular-nums text-slate-200">{item.required_qty} <span className="font-normal text-slate-500">{item.stock_uom}</span></p>
            </div>
            <div>
              <p className="text-slate-500">Transferred</p>
              <p className="font-bold tabular-nums text-slate-200">{item.transferred_qty}</p>
            </div>
            <div>
              <p className="text-slate-500">Remaining</p>
              <p className={`font-bold tabular-nums ${item.remaining_transfer_qty > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>
            ...
            <div>
              ...
              <p className={`font-bold tabular-nums ${
                item.available_qty >= item.remaining_transfer_qty ? 'text-emerald-400' :
                item.available_qty > 0 ? 'text-amber-400' : 'text-red-400'
          ...
          <div className="flex flex-wrap gap-1">
            {item.alternatives.map(alt => (
              <button
                ...
                className="text-[9px] bg-slate-700 hover:bg-slate-600 text-slate-200 rounded px-2 py-0.5 cursor-pointer transition-colors"
              >
...
export default function WorkOrderDetailModal({
  ...
  const statusBadge = wo ? (STATUS_COLORS[wo.status] ?? 'bg-slate-600') : 'bg-slate-600'
    ...
    <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-3 px-4 py-3 bg-slate-800 border-b border-slate-700 shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <button onClick={onClose} className="text-slate-400 hover:text-white transition-colors cursor-pointer shrink-0">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-slate-400">{woName}</span>
              {wo && (
                <span className={`text-[9px] font-bold text-white rounded px-1.5 py-px leading-none ${statusBadge}`}>
                  {wo.status}
                </span>
              )}
            </div>
            {wo && (
              <p className="text-sm font-bold text-white truncate">{wo.item_name || wo.production_item}</p>
            )}
          </div>
        </div>
        <button
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-[9px] text-slate-400">Available at WIP</span>
                <span className={`text-[9px] tabular-nums font-bold ${
                  wo.per_available >= 100 ? 'text-emerald-400' : wo.per_available > 0 ? 'text-amber-400' : 'text-red-400'
            ...
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-[9px] text-slate-400">Produced</span>
                <span className="text-[9px] tabular-nums text-slate-300">{wo.produced_qty} / {wo.qty}</span>
              </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div>
            {/* Column header */}
            <div className="grid grid-cols-4 gap-1 px-5 py-1 bg-slate-800 border-b border-slate-700 text-[9px] font-semibold text-slate-400 uppercase tracking-wider sticky top-0">
            ...
            {wo.status === 'Completed' && (
              <div className="flex items-center gap-2 mx-3 my-4 p-3 bg-emerald-900/30 border border-emerald-700/50 rounded text-emerald-400">
      ...
      {wo && wo.status !== 'Completed' && wo.status !== 'Stopped' && (
        <div className="shrink-0 bg-slate-800 border-t border-slate-700 px-4 py-3 flex flex-wrap gap-2">
          {hasRemainingTransfer && (
            <button
              onClick={() => onTransferMaterials(wo)}
              className="flex items-center gap-1.5 bg-blue-700 hover:bg-blue-600 active:bg-blue-800 text-white text-[11px] font-bold rounded px-3 py-2 transition-colors cursor-pointer"
            >
          ...
          {canManufacture && (
            <button
              ...
              className="flex items-center gap-1.5 bg-emerald-700 hover:bg-emerald-600 active:bg-emerald-800 text-white text-[11px] font-bold rounded px-3 py-2 transition-colors cursor-pointer"
            >
          ...
          {hasShortfall && (
            <button
              ...
              className="flex items-center gap-1.5 bg-amber-700 hover:bg-amber-600 active:bg-amber-800 text-white text-[11px] font-bold rounded px-3 py-2 transition-colors cursor-pointer"
            >
